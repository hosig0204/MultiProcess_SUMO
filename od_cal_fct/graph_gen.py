...
def xml2df_edge_flow_v2 (in_str_path_edgeInfo:str):   
    ...
    # Reset Index.    
    df_concat_rows_tmp1.set_index("edge_id", inplace= True, drop= True)
        
    return df_concat_rows_tmp1
...
# Function to create flow dataframe for Munich Motorway Network.
def seg_edge_ordered_flow (in_df_edge_seg_ordered:pd.DataFrame, in_df_edge_flow_raw:pd.DataFrame) -> pd.DataFrame:
    
    df_edge_in = in_df_edge_seg_ordered.copy()
    df_edge_in["edge_flow"] = np.nan
    
    for idx in df_edge_in.index:
        edge_id_tmp1 = df_edge_in.at[idx, "edge_id"]
        edge_type_tmp1 = df_edge_in.at[idx, "edge_type"]
        if edge_type_tmp1 != "segment":
            df_edge_in.at[idx, "edge_flow"] = in_df_edge_flow_raw.at[edge_id_tmp1, "edge_flow"]
        else:
            lst_seg_edges_tmp1 = str2list(df_edge_in.at[idx, "seg_edges"])
            lst_flow_seg_edges_tmp1 = list(map(lambda x: in_df_edge_flow_raw.at[x, "edge_flow"], lst_seg_edges_tmp1))
            fl_mean_flow_seg_edges_tmp1 = statistics.fmean(lst_flow_seg_edges_tmp1)
            df_edge_in.at[idx, "edge_flow"] = fl_mean_flow_seg_edges_tmp1
            
    df_edge_in["edge_flow"] = df_edge_in["edge_flow"].fillna(0.0)
    
    df_edge_out = df_edge_in[["edge_id", "edge_flow"]].copy()
    df_edge_out.set_index("edge_id", inplace= True, drop= True)
